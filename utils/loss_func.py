...
class NLLRankSurvLoss(nn.Module):
    def __init__(self, alpha=0.0, ranking_weight=0.8, nll_weight = 0.2, eps=1e-7, reduction='mean', phi='sigmoid'):
        super().__init__()
        self.alpha = alpha # weigh uncensored
        self.eps = eps
        self.reduction = reduction # sum or mean
        self.phi = phi # could be sigmoid or relu
        # self.rank_loss = SurvRankingLoss()
    ...
    def __call__(self, h, y, t, c):
        ...
        risk = torch.Tensor(risk).unsqueeze(1)
        # rank_loss = self.rank_loss(z = risk, censorship = c, times = t)
        rank_loss = ranking_loss(risks=risk, times=t, c=c, phi=self.phi, reduction=self.reduction, device=self.device)

        nll_loss_value = nll_loss(h=h, y=y.unsqueeze(dim=1), c=c.unsqueeze(dim=1),
            alpha=self.alpha, eps=self.eps,
            reduction=self.reduction)
        return self.ranking_weight * rank_loss + self.nll_weight * nll_loss_value


def ranking_loss(risks, times, c, phi, reduction, device):
